...
LR35902::LR35902(Bus& bus)
	...
	bus(bus)
{
	GetRegister16(REGISTER_AF) = 0x0000;
	GetRegister16(REGISTER_BC) = 0x0000;
	GetRegister16(REGISTER_DE) = 0x0000;
	GetRegister16(REGISTER_HL) = 0x0000;
	PC = 0x0100;
	SP = 0xFFFE;
}
...
void LR35902::Tick()
{
	...
	if (flag != 0)
	{
		...
		if (interrupt)
		{
			...
			uint8_t mask;
			SP--;
			bus.Write(SP, (PC & 0xFF00) >> 8);
			flag = bus.Read(0xFFFF) & bus.Read(0xFF0F) & 0x1F;
			if ((flag & 0x01) != 0)
			{
				addr = 0x0040;
				mask = 0x01;
	...
	if (!stop && !halt)
	{
		if (sleep <= 0)
		{
			...
			case 0x27:// DAA
			{
				...
				if (GetFlag(FLAG_N) == 0)
				{
					if (GetFlag(FLAG_C) == FLAG_SET || r > 0x99)
					{
						r += 0x60;
						SetFlag(FLAG_C, FLAG_SET);
					}
					if (GetFlag(FLAG_H) == FLAG_SET || (r & 0x0F) > 0x09)
					{
						r += 0x06;
					}
				}
				...
				{
					if (GetFlag(FLAG_C) == FLAG_SET)
					{
						r -= 0x60;
					}
					if (GetFlag(FLAG_H) == FLAG_SET)
					{
						r -= 0x06;
					}
				}
			...
			case 0x35:// DEC (HL)
			{
				uint8_t a = (bus.Read(GetRegister16(REGISTER_HL))) - 1;
				bus.Write(GetRegister16(REGISTER_HL), a);
				SetFlag(FLAG_Z, a == 0 ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_N, FLAG_SET);
				SetFlag(FLAG_H, (a & 0x0f) == 0x0F ? FLAG_SET : FLAG_CLEAR);
				sleep = 12;
			}
			break;
			case 0x36:// LD (HL),d8
			...
			{
				auto old = GetRegister(REGISTER_A);
				auto add = bus.Read(GetRegister16(REGISTER_HL));
				auto c = GetFlag(FLAG_C) & 0x01;
				auto& res = GetRegister(REGISTER_A);
				res += c + add;
				SetFlag(FLAG_Z, res == 0 ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_H, (old & 0x0F) + (add & 0x0F) + c > 0x0F ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_C, old + add + c > 0xFF ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_N, FLAG_CLEAR);
				sleep = 8;
			}
			break;
			...
			{
				auto old = GetRegister(REGISTER_A);
				auto sub = bus.Read(GetRegister16(REGISTER_HL));
				auto c = GetFlag(FLAG_C) & 0x01;
				auto& res = GetRegister(REGISTER_A);
				res = old - (c + sub);
				SetFlag(FLAG_Z, res == 0 ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_H, (old & 0x0F) < (c + (sub & 0x0F)) ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_C, old < (sub + c) ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_N, FLAG_SET);
				sleep = 8;
			}
				break;
			...
			case 0xE8:// ADD SP,r8
				{ 
					...
					SetFlag(FLAG_H, (old & 0x000F) > (res & 0x000F) ? FLAG_SET : FLAG_CLEAR);
					SetFlag(FLAG_C, (old & 0x00FF) > (res & 0x00FF) ? FLAG_SET : FLAG_CLEAR);
					SetFlag(FLAG_N, FLAG_CLEAR);
					sleep = 16;
				}
				break;
			...
			{
				auto old = SP;
				auto res = old + int8_t(Fetch());
				SetFlag(FLAG_Z, FLAG_CLEAR);
				SetFlag(FLAG_H, (old & 0x000F) > (res & 0x000F) ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_C, (old & 0x00FF) > (res & 0x00FF) ? FLAG_SET : FLAG_CLEAR);
				SetFlag(FLAG_N, FLAG_CLEAR);
				GetRegister16(REGISTER_HL) = res;
				sleep = 12;
			}
				break;
			case 0xF9:// LD SP,HL
...
{
	auto old = GetRegister(lhs);
	auto add = GetRegister(rhs);
	auto c = GetFlag(FLAG_C) & 0x01;
	auto& res = GetRegister(lhs);
	res += c + add;
	SetFlag(FLAG_Z, res == 0 ? FLAG_SET : FLAG_CLEAR);
	SetFlag(FLAG_H, (old & 0x0F) + (add & 0x0F) + c > 0x0F ? FLAG_SET : FLAG_CLEAR);
	SetFlag(FLAG_C, old + add + c > 0xFF ? FLAG_SET : FLAG_CLEAR);
...
void LR35902::Restart(uint16_t addr)
{
	SP -= 2;
	bus.Write16(SP, PC);
	PC = addr;
